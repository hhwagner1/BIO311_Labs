...
```
Interactively select at least 5 points with tree cover

```{r}
Tree.points <- viewRGB(sentinel2_12Sept2024_brick, r = 4, g = 3, b = 2) %>% 
  editMap()
Tree.points <- Tree.points$finished$geometry %>% st_sf() %>% mutate(class = "Tree", id = 1)
```

Repeat for roads

```{r}
Road.points <- Tree.points %>% filter(is.na(id))
Road.points <- viewRGB(sentinel2_12Sept2024_brick, r = 4, g = 3, b = 2) %>% 
  editMap()
Road.points <- Road.points$finished$geometry %>% st_sf() %>% mutate(class = "Road", id = 3)
```

Repeat for turf (tree-less green area)

```{r}
Turf.points <- Tree.points %>% filter(is.na(id))
Turf.points <- viewRGB(sentinel2_12Sept2024_brick, r = 4, g = 3, b = 2) %>% 
  editMap()
Turf.points <- Turf.points$finished$geometry %>% st_sf() %>% mutate(class = "Turf", id = 2)
```

Repeat for buildings

```{r}
Building.points <- Tree.points %>% filter(is.na(id))
Building.points <- viewRGB(sentinel2_12Sept2024_brick, r = 4, g = 3, b = 2) %>% 
  editMap()
Building.points <- Building.points$finished$geometry %>% st_sf() %>% mutate(class = "Building", id = 4)
```

Repeat for reflections?

```{r}
Reflect.points <- Tree.points %>% filter(is.na(id))
Reflect.points <- viewRGB(sentinel2_12Sept2024_brick, r = 4, g = 3, b = 2) %>% 
  editMap()
Reflect.points <- Reflect.points$finished$geometry %>% st_sf() %>% mutate(class = "Reflection", id = 5)
```

Repeat for water (if present)

```{r}
Water.points <- viewRGB(sentinel2_12Sept2024_brick, r = 4, g = 3, b = 2) %>% 
  editMap()
if(length(Water.points$drawn) > 0)
{
  Water.points <- Water.points$finished$geometry %>% st_sf() %>% mutate(class = "Cloud", id = 9)
}else{
  Water.points <- Tree.points %>% filter(is.na(id))
}
```
...
```{r}
Cloud.points <- viewRGB(sentinel2_12Sept2024_brick, r = 4, g = 3, b = 2) %>% 
  editMap()
if(length(Cloud.points$drawn) > 0)
{
  Cloud.points <- Cloud.points$finished$geometry %>% st_sf() %>% mutate(class = "Cloud", id = 9)
}else{
  Cloud.points <- Tree.points %>% filter(is.na(id))
}
```

```{r}
training_points <- rbind(Tree.points, Turf.points, Road.points, Building.points, Reflect.points, Water.points, Cloud.points)
