...
- Delineate landscapes with different characters (Ecolandscapes approach)
...
- The ratio of the two ($# forest cells / # cells with non-missing values$) gives the proportion of cells with non-missing values that are forested. Mathematically, this is the same as the mean of the variable as long as the variable is coded 1/0. 
...
An equivalent approach is to use terra's `global` function:

```{r}
Forest.percent.Method2 <- terra::global(Forest.r10, fun="mean", na.rm=TRUE)
Forest.percent.Method2
```

And a third way is to calculate the landscape metric 'percent of landscape' with the `landscape metrics` R package. 

```{r}
Forest.percent.Method3 <- landscapemetrics::lsm_c_pland(Forest.r10)
Forest.percent.Method3
```
Note: this function calculated pland for all categories in the dataset. Recall that 0 = non-forest and 1 = forest. The first row here is for non-forest, the second row for forest.

All three methods returned the same value and there was no real advantage in using the `landscapemetrics` package. However, if we want to evaluate spatial configuration, rather than composition, we will need the `landscapemetrics` package.

## d) Determine landscape composition

Let's stick with landscape composition a little longer, though, and calculate `pland` (percent of landscape) of all ELC types in your neighbourhood. To interpret the results, we need to join the resulting table with the category labels. Also, we will repeat the analysis for 10m and 30 m resolution and put everything in a single results table.
...
Hint: if you don't want to click through the pages, you can try one if these:

- in the console, type `View(Results)` to open the data frame in a tab, or
- in the console, type `head(Results, nrow(Results))` to print all rows in the console.
